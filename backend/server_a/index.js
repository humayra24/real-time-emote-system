...
const processMessage = async ({ topic, partition, message }) => {
  try {
    if (topic === "video-stream") {
      ...
      console.log("Received video chunk from kafka:", data.index);
      // TESTING

      // Handle video chunks
      // const chunk = message.value;
      // const index = message.headers.index
      //   ? message.headers.index.toString()
      //   : "0";
...
const run = async () => {
  try {
    ...
    await consumer.subscribe({
      topics: ["video-stream", "aggregated-emote-data"],
      fromBeginning: true,
    });
